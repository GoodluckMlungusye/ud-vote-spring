...
public class ContestantService {
    ...
    public String addContestant(ContestantDTO contestantDTO, MultipartFile file) {
        ...
        Student student = studentRepository.findById(contestantDTO.getStudentId())
                .orElseThrow(() -> new IllegalArgumentException("Student not found"));
        Category category = categoryRepository.findById(contestantDTO.getCategoryId())
        ...
        return "Contestant " + savedContestant.getStudent().getUser().getFirstName() +
                " " + savedContestant.getStudent().getUser().getLastName() + " has been added successfully";
    }
    ...
    public String updateContestant(Long contestantId, ContestantDTO contestantDTO, MultipartFile file) {
        Contestant existingContestant = contestantRepository.findById(contestantId)
                .orElseThrow(() -> new IllegalArgumentException("Contestant was not found"));
        ...
        if (contestantDTO.getStudentId() != null &&
                !contestantDTO.getStudentId().equals(existingContestant.getStudent().getId())) {
            Student newStudent = studentRepository.findById(contestantDTO.getStudentId())
                    .orElseThrow(() -> new IllegalArgumentException("Student does not exist"));
            existingContestant.setStudent(newStudent);
        }

        if (contestantDTO.getCategoryId() != null &&
                !contestantDTO.getCategoryId().equals(existingContestant.getCategory().getId())) {
            Category newCategory = categoryRepository.findById(contestantDTO.getCategoryId())
        ...
        return "Contestant " + updatedContestant.getStudent().getUser().getFirstName() +
                " " + updatedContestant.getStudent().getUser().getLastName() + " has been updated successfully";
    }

    public String deleteContestant(Long contestantId) {
        Contestant contestant = contestantRepository.findById(contestantId)
                .orElseThrow(() -> new IllegalArgumentException("Contestant does not exist"));

        fileService.deleteFile(contestant.getVideoUrl());
        contestantRepository.deleteById(contestantId);

        return "Contestant " + contestant.getStudent().getUser().getFirstName() +
                " " + contestant.getStudent().getUser().getLastName() + " has been deleted";
    }
}
